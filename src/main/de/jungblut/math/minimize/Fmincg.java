...
public final class Fmincg implements Minimizer {
	...
	/**
	 * Minimizes the given CostFunction with Nonlinear conjugate gradient
	 * method. <br/>
	 ...
	 * @param pInput
	 *            the input vector, also called starting point
	 * @param length
	...
	public static DoubleVector minimizeFunction(CostFunction f,
		...
		while (i < Math.abs(length)) {// while not finished
			...
			if (success == 1) { // if line search succeeded
				...
				final double numerator = (df2.dot(df2) - df1.dot(df2))
						/ df1.dot(df1);
				...
				if (ls_failed == 1 || i > Math.abs(length)) {
					System.out.println("Line search FAILED after " + i
							+ " iterations");
